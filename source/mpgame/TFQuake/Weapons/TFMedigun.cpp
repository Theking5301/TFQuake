...
class TFMedigun : public rvWeapon {
public:

	CLASS_PROTOTYPE( TFMedigun );
...
protected:
	...
	TFMedigunPath						currentPath;
	
	// Chain lightning mod
	idList<TFMedigunPath>				chainLightning;
...
private:
	...
	CLASS_STATES_PROTOTYPE ( TFMedigun );
};

CLASS_DECLARATION( rvWeapon, TFMedigun )
EVENT( EV_Lightninggun_RestoreHum,			TFMedigun::Event_RestoreHum )
END_CLASS
...
void TFMedigun::Spawn( void ) {
	...
	//SD BEGIN
	healPerSecond   = spawnArgs.GetFloat( "heal_per_second", "10");
	uberAddTickRate   = spawnArgs.GetFloat( "uber_add_tick_rate", "1");
	uberUseTickRate   = spawnArgs.GetFloat( "uber_use_tick_rate", "1");
	uberchargeTimer = 0;
	bIsInUbermode = false;
	zoomFov = 100;
...
================
*/
void TFMedigun::Save	( idSaveGame* savefile ) const {
	int i;
...
================
*/
void TFMedigun::Restore ( idRestoreGame* savefile ) {
	int   i;
	int   num;
	float f;
...
void TFMedigun::Think ( void ) {
	...
	if ((!wsfl.attack && !wsfl.zoom && !bIsInUbermode) || !IsReady() || !AmmoAvailable() ) {
		if ( trailEffectView ) {
			trailEffectView->Stop ( );
			trailEffectView = NULL;
		}
...
// RAVEN END
	...
	if (!bIsInUbermode && !wsfl.zoom && !gameLocal.isClient && gameLocal.time >= nextAttackTime ) {
		int    i;
		float  power = 1.0f;
		idVec3 dir;
	...
	if (!wsfl.zoom && gameLocal.time > nextCrawlTime ) {
...
================
*/
void TFMedigun::Attack ( idEntity* ent, const idVec3& dir, float power ) {
	...
	if(!bIsInUbermode) {
		...
		if(ent) {
			idPlayer* tempPlayer = (idPlayer*)ent;
			if(tempPlayer && tempPlayer->team == owner->team) {
...
/*
================
TFMedigun::UpdateChainLightning
================
*/
void TFMedigun::UpdateChainLightning ( void ) {
	int					i;
	TFMedigunPath*	parent;
	TFMedigunPath*	path;
	idActor*			target;
...
/*
================
TFMedigun::UpdateEffects
================
*/
void TFMedigun::UpdateEffects( const idVec3& origin ) {
	int					i;
	TFMedigunPath*	parent;
	idVec3				dir;
...
/*
================
TFMedigun::UpdateTrailEffect
================
*/
void TFMedigun::UpdateTrailEffect( rvClientEffectPtr& effect, const idVec3& start, const idVec3& end, bool view ) {
	idVec3 dir;
	dir = end - start;
	dir.Normalize();
...
/*
================
TFMedigun::StopChainLightning
================
*/
void TFMedigun::StopChainLightning( void ) {
	int	i;
...
/*
================
TFMedigun::ClientStale
================
*/
void TFMedigun::ClientStale( void ) {
...
/*
================
TFMedigun::UpdateTubes
================
*/
void TFMedigun::UpdateTubes( void ) {
	idAnimator* animator;
...
CLASS_STATES_DECLARATION ( TFMedigun )
	STATE ( "Raise",						TFMedigun::State_Raise )
	STATE ( "Lower",						TFMedigun::State_Lower )
	STATE ( "Idle",							TFMedigun::State_Idle)
	STATE ( "Fire",							TFMedigun::State_Fire )
END_CLASS_STATES

/*
================
TFMedigun::State_Raise
================
*/
stateResult_t TFMedigun::State_Raise( const stateParms_t& parms ) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};	
...
/*
================
TFMedigun::State_Lower
================
*/
stateResult_t TFMedigun::State_Lower( const stateParms_t& parms ) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
		STAGE_WAITRAISE
...
/*
================
TFMedigun::State_Idle
================
*/
stateResult_t TFMedigun::State_Idle( const stateParms_t& parms ) {
	enum {
		STAGE_INIT,
		STAGE_WAIT,
	};	
...
/*
================
TFMedigun::State_Fire
================
*/
stateResult_t TFMedigun::State_Fire( const stateParms_t& parms ) {
	enum {
		STAGE_INIT,
		STAGE_ATTACKLOOP,
		STAGE_DONE,
...
/*
================
TFMedigun::Event_RestoreHum
================
*/
void TFMedigun::Event_RestoreHum ( void ) {
	StopSound( SND_CHANNEL_BODY3, false );
	StartSound( "snd_idle_hum", SND_CHANNEL_BODY3, 0, false, NULL );
}
...
/*
================
TFMedigun::ClientUnStale
================
*/
void TFMedigun::ClientUnstale( void ) {
	Event_RestoreHum();
}
...
/*
================
TFMedigunPath::StopEffects
================
*/
void TFMedigunPath::StopEffects( void ) {
	if ( trailEffect ) {
		trailEffect->Stop( );
...
/*
================
TFMedigunPath::UpdateEffects
================
*/
void TFMedigunPath::UpdateEffects ( const idVec3& from, const idDict& dict ) {
	idVec3 dir;
	dir = origin - from;
	dir.Normalize();
...
================
*/
void TFMedigunPath::Save( idSaveGame* savefile ) const {
...
/*
================
TFMedigunPath::Restore
================
*/
void TFMedigunPath::Restore( idRestoreGame* savefile ) {
