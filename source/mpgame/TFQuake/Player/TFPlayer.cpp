...
CLASS_DECLARATION( idActor, TFPlayer )
//	EVENT( EV_Player_HideDatabaseEntry,		TFPlayer::Event_HideDatabaseEntry )
	EVENT( EV_Player_ZoomIn,				TFPlayer::Event_ZoomIn )
	EVENT( EV_Player_ZoomOut,				TFPlayer::Event_ZoomOut )
	EVENT( EV_Player_GetButtons,			TFPlayer::Event_GetButtons )
	EVENT( EV_Player_GetMove,				TFPlayer::Event_GetMove )
	EVENT( EV_Player_GetViewAngles,			TFPlayer::Event_GetViewAngles )
	EVENT( EV_Player_SetViewAngles,			TFPlayer::Event_SetViewAngles )
	EVENT( EV_Player_StopFxFov,				TFPlayer::Event_StopFxFov )
	EVENT( EV_Player_EnableWeapon,			TFPlayer::Event_EnableWeapon )
	EVENT( EV_Player_DisableWeapon,			TFPlayer::Event_DisableWeapon )
	EVENT( EV_Player_GetCurrentWeapon,		TFPlayer::Event_GetCurrentWeapon )
	EVENT( EV_Player_GetPreviousWeapon,		TFPlayer::Event_GetPreviousWeapon )
	EVENT( EV_Player_SelectWeapon,			TFPlayer::Event_SelectWeapon )
	EVENT( EV_Player_GetWeaponEntity,		TFPlayer::Event_GetWeaponEntity )
	EVENT( EV_Player_ExitTeleporter,		TFPlayer::Event_ExitTeleporter )
	EVENT( EV_Player_HideTip,				TFPlayer::Event_HideTip )
	EVENT( EV_Player_LevelTrigger,			TFPlayer::Event_LevelTrigger )
	EVENT( EV_Player_GetViewPos,			TFPlayer::Event_GetViewPos )
	EVENT( EV_Player_FinishHearingLoss,		TFPlayer::Event_FinishHearingLoss )
	EVENT( EV_Player_GetAmmoData,			TFPlayer::Event_GetAmmoData )
	EVENT( EV_Player_RefillAmmo,			TFPlayer::Event_RefillAmmo )
	EVENT( EV_Player_AllowFallDamage,		TFPlayer::Event_AllowFallDamage )


// mekberg: allow enabling/disabling of objectives
	EVENT ( EV_Player_EnableObjectives,		TFPlayer::Event_EnableObjectives )
	EVENT ( EV_Player_DisableObjectives,	TFPlayer::Event_DisableObjectives )

// mekberg: don't suppress showing of new objectives anymore
	EVENT ( EV_Player_AllowNewObjectives,	TFPlayer::Event_AllowNewObjectives )
// RAVEN END
	
	EVENT( AI_EnableTarget,					TFPlayer::Event_EnableTarget )
	EVENT( AI_DisableTarget,				TFPlayer::Event_DisableTarget )

	EVENT( EV_ApplyImpulse,					TFPlayer::Event_ApplyImpulse )

// RAVEN BEGIN
// mekberg: sethealth on player.
	EVENT( AI_SetHealth,					TFPlayer::Event_SetHealth )
//MCG: setArmor
	EVENT( EV_Player_SetArmor,				TFPlayer::Event_SetArmor )
// RAVEN END;
	EVENT( EV_Player_SetExtraProjPassEntity,TFPlayer::Event_SetExtraProjPassEntity )
//MCG: direct damage
	EVENT( EV_Player_DamageEffect,			TFPlayer::Event_DamageEffect )
END_CLASS
...
==============
*/
void idInventory::GivePowerUp( TFPlayer *player, int powerup, int msec ) {
	powerups |= 1 << powerup;
...
==============
*/
void idInventory::RestoreInventory( TFPlayer *owner, const idDict &dict ) {
	int			i;
	int			num;
	idDict		*item;
	idStr		key;
...
==============
*/
int idInventory::MaxAmmoForAmmoClass( TFPlayer *owner, const char *ammo_classname ) const {
...
==============
*/
bool idInventory::DetermineAmmoAvailability( TFPlayer* owner, const char *ammoName, int ammoIndex, int ammoAmount, int ammoMax ) {
...
==============
*/
bool idInventory::Give( TFPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud, bool dropped, bool checkOnly ) {
	int						i;
	const char				*pos;
	const char				*end;
	int						len;
...
==============
*/
void TFPlayer::SetShowHud( bool showHud )	{
	disableHud = !showHud;
}
...
==============
*/
void TFPlayer::SetWeapon( int weaponIndex ) {
...
/*
==============
TFPlayer::SetupWeaponEntity
==============
*/
void TFPlayer::SetupWeaponEntity( void ) {
	int						w;
	const char				*weap;
...
// jnewquist: Use accessor for static class type 
		...
		if ( !spawn ) {
			gameLocal.Error( "TFPlayer::SetupWeaponEntity: failed to spawn weaponViewModel" );
		}
	...
	if ( !weaponWorldModel ) {
		...
		gameLocal.SpawnEntityDef( args, &spawn );
		if ( !spawn ) {
			gameLocal.Error( "TFPlayer::SetupWeaponEntity: failed to spawn weaponWorldModel" );
		}
...
/*
===============
TFPlayer::ProjectHeadOverlay
===============
*/
void TFPlayer::ProjectHeadOverlay( const idVec3 &point, const idVec3 &dir, float size, const char *decal ) {
...
/*
===============
TFPlayer::GetCursorGUI
===============
*/
idUserInterface* TFPlayer::GetCursorGUI( void ) {
	idStr temp;
...
void TFPlayer::Spawn( void ) {
	...
	if ( !gameLocal.isMultiplayer || IsLocalClient() ) {
		...
		if ( spawnArgs.GetString( "hud", "", temp ) ) {
			...
			gameLocal.Warning( "TFPlayer::Spawn() - No hud for player." );
		}
		...
		if ( gameLocal.isMultiplayer ) {
			if ( spawnArgs.GetString( "mphud", "", temp ) ) {
				...
				gameLocal.Warning( "TFPlayer::Spawn() - No MP hud overlay while in MP.");
			}
		}
	...
	if ( gameLocal.isMultiplayer ) {
		...
		// Normally TFPlayer::Move() gets called to set the contents to 0, but we don't call
...
===========
*/
void TFPlayer::Save( idSaveGame *savefile ) const {
	assert( !IsFakeClient() );
...
===========
*/
void TFPlayer::Restore( idRestoreGame *savefile ) {
	assert( !IsFakeClient() );
...
/*
===============
TFPlayer::PrepareForRestart
================
*/
void TFPlayer::PrepareForRestart( void ) {
	ClearPowerUps();
	Spectate( true );
...
/*
===============
TFPlayer::ServerSpectate
================
*/
void TFPlayer::ServerSpectate( bool spectate ) {
	assert( !gameLocal.isClient );
...
/*
===========
TFPlayer::SelectSpawnPoint

Find a spawn point marked, otherwise use normal spawn selection.
============
*/
bool TFPlayer::SelectSpawnPoint( idVec3 &origin, idAngles &angles ) {
	idEntity *spot;
	idStr skin;
...
/*
===========
TFPlayer::SpawnFromSpawnSpot

Chooses a spawn location and spawns the player
============
*/
void TFPlayer::SpawnFromSpawnSpot( void ) {
	idVec3		spawn_origin;
...
============
*/
void TFPlayer::SpawnToPoint( const idVec3 &spawn_origin, const idAngles &spawn_angles ) {
	idVec3 spec_origin;
...
/*
===============
TFPlayer::SavePersistantInfo

Saves any inventory and player stats when changing levels.
===============
*/
void TFPlayer::SavePersistantInfo( void ) {
	if ( IsFakeClient() ) {
		return;
	}
...
/*
===============
TFPlayer::RestorePersistantInfo

Restores any inventory and player stats when changing levels.
===============
*/
void TFPlayer::RestorePersistantInfo( void ) {
	if ( IsFakeClient() ) {
		return;
	}
...
/*
================
TFPlayer::GetUserInfo
================
*/
idDict *TFPlayer::GetUserInfo( void ) {
	if ( IsFakeClient() ) {
		return NULL;
	}
...
==============
*/
void TFPlayer::UpdateModelSetup( bool forceReload ) {
	const idDict* dict;
	...
	if( !model ) {
		...
		if( !model ) {
			gameLocal.Error( "TFPlayer::UpdateModelSetup() - Can't find default model (%s)\n", defaultModel );
		} else {
...
void TFPlayer::UpdateSounds(const rvDeclPlayerModel *modelDecl)
{
...
/*
==============
TFPlayer::BalanceTeam
==============
*/
bool TFPlayer::BalanceTeam( void ) {
	assert( !IsFakeClient() );
	...
	for( i = 0; i < gameLocal.numClients; i++ ) {
		...
		if ( ent && ent->IsType( TFPlayer::Type ) && gameLocal.mpGame.IsInGame( i ) ) {
			if ( !static_cast< TFPlayer * >( ent )->spectating ) {
				teamCount[ static_cast< TFPlayer * >( ent )->team ]++;
			}
		}
	}
...
/*
==============
TFPlayer::UserInfoChanged
==============
*/
bool TFPlayer::UserInfoChanged( void ) {
 	idDict	*userInfo;
 	bool	modifiedInfo;
 	bool	spec;
 	bool	newready;
...
/*
===============
TFPlayer::UpdateHudAmmo
===============
*/
void TFPlayer::UpdateHudAmmo( idUserInterface *_hud ) {
	int inclip;
	int ammoamount;
...
/*
===============
TFPlayer::UpdateHudStats
===============
*/
void TFPlayer::UpdateHudStats( idUserInterface *_hud ) {
	int temp;
...
/*
===============
TFPlayer::UpdateHudWeapon
===============
*/
void TFPlayer::UpdateHudWeapon( int displayWeapon ) {
	...
	int index = 0;
	int idealIndex = 0;
	idUserInterface * hud = TFPlayer::hud;
	idUserInterface * mphud = TFPlayer::mphud;
	idUserInterface * cursor = TFPlayer::cursor;

	TFPlayer *p = gameLocal.GetLocalPlayer();
...
void TFPlayer::UpdateHudPowerUps( idUserInterface *_hud ) {
	assert( _hud );
...
/*
===============
TFPlayer::StartRadioChatter
===============
*/
void TFPlayer::StartRadioChatter ( void ) {
	if ( hud ) {
...
/*
===============
TFPlayer::StopRadioChatter
===============
*/
void TFPlayer::StopRadioChatter ( void ) {
	if ( hud ) {
...
===============
*/
void TFPlayer::DrawShadow( renderEntity_t *headRenderEnt ) {
...
===============
*/
void TFPlayer::DrawHUD( idUserInterface *_hud ) {
	idUserInterface * cursor = TFPlayer::cursor;
...
/*
===============
TFPlayer::EnterCinematic
===============
*/
void TFPlayer::EnterCinematic( void ) {
	Hide();
...
/*
===============
TFPlayer::ExitCinematic
===============
*/
void TFPlayer::ExitCinematic( void ) {
  	Show();
...
/*
===============
TFPlayer::SkipCinematic
===============
*/
bool TFPlayer::SkipCinematic( void ) {
...
/*
=====================
TFPlayer::UpdateConditions
=====================
*/
void TFPlayer::UpdateConditions( void ) {
	idVec3	velocity;
	float	fallspeed;
	float	forwardspeed;
	float	sidespeed;
...
/*
==================
TFPlayer::WeaponFireFeedback

Called when a weapon fires, generates head twitches, etc
==================
*/
void TFPlayer::WeaponFireFeedback( const idDict *weaponDef ) {
	// force a blink
	blink_time = 0;
...
/*
===============
TFPlayer::CacheWeapons
===============
*/
void TFPlayer::CacheWeapons( void ) {
	idStr	weap;
	int		w;
...
===============
*/
bool TFPlayer::Give( const char *statname, const char *value, bool dropped ) {
	int amount;
...
===============
*/
bool TFPlayer::GiveItem( idItem *item ) {
	int					i;
...
/*
===============
TFPlayer::PowerUpModifier
===============
*/
float TFPlayer::PowerUpModifier( int type ) {
	float mod = 1.0f;
...
/*
===============
TFPlayer::PowerUpActive
===============
*/
bool TFPlayer::PowerUpActive( int powerup ) const {
	return ( inventory.powerups & ( 1 << powerup ) ) != 0;
}

/*
===============
TFPlayer::StartPowerUpEffect
===============
*/
void TFPlayer::StartPowerUpEffect( int powerup ) {
...
/*
===============
TFPlayer::StopPowerUpEffect
===============
*/
void TFPlayer::StopPowerUpEffect( int powerup ) {
...
===============
*/
bool TFPlayer::GivePowerUp( int powerup, int time, bool team ) {
...
/*
==============
TFPlayer::ClearPowerup
==============
*/
void TFPlayer::ClearPowerup( int i ) {
	if ( gameLocal.isServer ) {
		idBitMsg	msg;
		byte		msgBuf[MAX_EVENT_PARAM_SIZE];
...
/*
==============
TFPlayer::GetArenaPowerupString
==============
*/
const char* TFPlayer::GetArenaPowerupString ( void ) {
	if ( PowerUpActive( POWERUP_SCOUT ) ) {
		return "^isct";
...
/*
==============
TFPlayer::UpdatePowerUps
==============
*/
void TFPlayer::UpdatePowerUps( void ) {
	int i;
	int wearoff = -1;
...
/*
===============
TFPlayer::ClearPowerUps
===============
*/
void TFPlayer::ClearPowerUps( void ) {
	int i;
 	for ( i = 0; i < POWERUP_MAX; i++ ) {
 		if ( PowerUpActive( i ) ) {
   			ClearPowerup( i );
...
/*
===============
TFPlayer::GiveWeaponMods
===============
*/
bool TFPlayer::GiveWeaponMods( int mods ) {
...
===============
*/
bool TFPlayer::GiveWeaponMods( int weapon, int mods ) {
...
/*
==============
TFPlayer::GiveWeaponMod
==============
*/
void TFPlayer::GiveWeaponMod ( const char* weaponmod ) {
...
/*
===============
TFPlayer::GiveInventoryItem
===============
*/
bool TFPlayer::GiveInventoryItem( idDict *item ) {
	if ( gameLocal.isMultiplayer && spectating ) {
		return false;
	}
...
/*
==============
TFPlayer::UpdateObjectiveInfo
==============
 */
void TFPlayer::UpdateObjectiveInfo( void ) {
...
/*
===============
TFPlayer::GiveObjective
===============
*/
void TFPlayer::GiveObjective( const char *title, const char *text, const char *screenshot ) {
...
/*
===============
TFPlayer::CompleteObjective
===============
*/
void TFPlayer::CompleteObjective( const char *title ) {
// RAVEN BEGIN
...
/*
===============
TFPlayer::FailObjective
===============
*/
void TFPlayer::FailObjective ( const char* title ) {
// RAVEN BEGIN
	title = common->GetLocalizedString( title );
...
/*
===============
TFPlayer::FindInventoryItem
===============
*/
idDict *TFPlayer::FindInventoryItem( const char *name ) {
...
/*
===============
TFPlayer::RemoveInventoryItem
===============
*/
void TFPlayer::RemoveInventoryItem( const char *name ) {
...
===============
*/
void TFPlayer::RemoveInventoryItem( idDict *item ) {
...
===============
*/
void TFPlayer::GiveItem( const char *itemname ) {
	idDict	args;
...
/*
==================
TFPlayer::SlotForWeapon
==================
*/
int TFPlayer::SlotForWeapon( const char *weaponName ) {
	int i;
...
/*
===============
TFPlayer::ScheduleWeaponSwitch
===============
*/
void TFPlayer::ScheduleWeaponSwitch(int weapon)
{
...
/*
===============
TFPlayer::ShowCrosshair
===============
*/
void TFPlayer::ShowCrosshair( void ) {
	if ( !weaponEnabled ) {
		return;
	}
...
/*
===============
TFPlayer::HideCrosshair
===============
*/
void TFPlayer::HideCrosshair( void ) {
	if ( cursor ) {
...
/*
===============
TFPlayer::NextBestWeapon
===============
*/
void TFPlayer::NextBestWeapon( void ) {
	const char *weap;
	int w = MAX_WEAPONS;
...
/*
===============
TFPlayer::SelectWeapon
===============
*/
void TFPlayer::SelectWeapon( const char *weapon_name ) {
...
===============
*/
void TFPlayer::SelectWeapon( int num, bool force ) {
	const char *weap;
...
=================
*/
idEntity* TFPlayer::DropItem( const char* itemClass, const idDict& customArgs, const idVec3& velocity ) const {
	idDict		args;
	idEntity*	ent;
...
/*
=================
TFPlayer::DropPowerups
=================
*/
void TFPlayer::DropPowerups( void ) {
	int			i;
	idEntity*	item;
...
=================
*/
idEntity* TFPlayer::ResetFlag ( const char* itemClass, const idDict& customArgs ) const {
	idDict		args;
	idEntity*	ent;
	args.Set( "classname", itemClass );
	args.Set( "origin", GetPhysics()->GetAbsBounds().GetCenter().ToString ( ) );
...
/*
=================
TFPlayer::RespawnFlags
=================
*/
void TFPlayer::RespawnFlags ( void ) {
	int			i;
	idEntity*	item;
...
===============
*/
idUserInterface *TFPlayer::ActiveGui( void ) {
#ifdef _XENON
	if ( objectiveSystemOpen ) {
		return 0;
	}
...
/*
===============
TFPlayer::Weapon_Combat
===============
*/
void TFPlayer::Weapon_Combat( void ) {
...
/*
===============
TFPlayer::Weapon_Vehicle
===============
*/
void TFPlayer::Weapon_Vehicle( void ) {
	StopFiring();
	weapon->LowerWeapon();
...
/*
===============
TFPlayer::Weapon_Usable
===============
*/
void TFPlayer::Weapon_Usable( void ) {
	StopFiring();
	weapon->LowerWeapon();
...
/*
===============
TFPlayer::LowerWeapon
===============
*/
void TFPlayer::LowerWeapon( void ) {
...
/*
===============
TFPlayer::RaiseWeapon
===============
*/
void TFPlayer::RaiseWeapon( void ) {
	if ( weapon && weapon->IsHidden() ) {
		weapon->RaiseWeapon();
	}
}
...
/*
===============
TFPlayer::WeaponLoweringCallback
===============
*/
void TFPlayer::WeaponLoweringCallback( void ) {
...
/*
===============
TFPlayer::WeaponRisingCallback
===============
*/
void TFPlayer::WeaponRisingCallback( void ) {
...
/*
===============
TFPlayer::UpdateWeapon
===============
*/
void TFPlayer::UpdateWeapon( void ) {
	if ( health <= 0 ) {
		return;
	}
...
/*
===============
TFPlayer::SpectateFreeFly
===============
*/
void TFPlayer::SpectateFreeFly( bool force ) {
	TFPlayer	*player;
	idVec3		newOrig;
	idVec3		spawn_origin;
	idAngles	spawn_angles;
...
/*
===============
TFPlayer::SpectateCycle
===============
*/
void TFPlayer::SpectateCycle( void ) {
	TFPlayer *player;
...
/*
===============
TFPlayer::UpdateSpectating
===============
*/
void TFPlayer::UpdateSpectating( void ) {
	assert( spectating );
	assert( !gameLocal.isClient || IsFakeClient() );
	assert( IsHidden() );
	TFPlayer *player;
...
/*
===============
TFPlayer::HandleSingleGuiCommand
===============
*/
bool TFPlayer::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src ) {
	idToken token;
...
==============
*/
bool TFPlayer::Collide( const trace_t &collision, const idVec3 &velocity ) {
	idEntity *other;
...
/*
================
TFPlayer::UpdateLocation

Searches nearby locations 
================
*/
 void TFPlayer::UpdateLocation( void ) {
   	if ( hud ) {
	...
	start = GetEyePosition();
 	...
 	if ( gameLocal.isMultiplayer && IsLocalClient() ) {
		...
		// no aim text if player is invisible
 		if ( ( trace.fraction < 1.0f ) && ( trace.c.entityNum < MAX_CLIENTS ) && ( !((TFPlayer*)gameLocal.entities[ trace.c.entityNum ])->PowerUpActive( POWERUP_INVISIBILITY ) ) ) {
			char* teammateHealth = "";
			TFPlayer* p = static_cast<TFPlayer*>(gameLocal.entities[ trace.c.entityNum ]);			
		...
		if ( gameLocal.isMultiplayer ) {
			...
			while ( ent != NULL ) {
				...
				if ( !ent->IsType(TFPlayer::GetClassType()) ) {
					isOk = false;
				} else if ( gameLocal.IsTeamGame() && ((TFPlayer*)ent)->team == team ) {
					isOk = false;
				} else if ( ((TFPlayer*)ent)->instance != instance ) {
					isOk = false;
				} else if ( ((TFPlayer*)ent)->spectating ) {
					isOk = false;
...
================
*/
void TFPlayer::ClearFocus ( void ) {
	SetFocus ( FOCUS_NONE, 0, NULL, NULL );
}

void TFPlayer::UpdateFocusCharacter( idEntity* newEnt ) {
	if ( !cursor ) {
		return;
	}
...
================
*/
void TFPlayer::SetFocus ( playerFocus_t newType, int _focusTime, idEntity* newEnt, idUserInterface* newUI ) {
	const char* command;
...
=================
*/
void TFPlayer::CrashLand( const idVec3 &oldOrigin, const idVec3 &oldVelocity ) {
...
===============
*/
void TFPlayer::BobCycle( const idVec3 &pushVelocity ) {
	float		bobmove;
	int			old, deltaTime;
...
/*
================
TFPlayer::UpdateDeltaViewAngles
================
*/
void TFPlayer::UpdateDeltaViewAngles( const idAngles &angles ) {
	// set the delta angle
	idAngles delta;
	for( int i = 0; i < 3; i++ ) {
...
/*
================
TFPlayer::SetViewAngles
================
*/
void TFPlayer::SetViewAngles( const idAngles &angles ) {
...
/*
================
TFPlayer::UpdateViewAngles
================
*/
void TFPlayer::UpdateViewAngles( void ) {
	int i;
	idAngles delta;
...
/*
==============
TFPlayer::UpdateGravity
==============
*/
void TFPlayer::UpdateGravity( void ) {
...
/*
==============
TFPlayer::ToggleObjectives
==============
*/
void TFPlayer::ToggleObjectives ( void ) {
// RAVEN BEGIN
...
/*
==============
TFPlayer::ToggleScoreboard
==============
*/
void TFPlayer::ToggleScoreboard( void ) {
	scoreBoardOpen ^= 1;
}
...
==============
*/
void TFPlayer::Spectate( bool spectate, bool force ) {
 	idBitMsg	msg;
 	byte		msgBuf[MAX_EVENT_PARAM_SIZE];
	...
	if ( spectating || inOtherInstance ) {
		...
		StopRagdoll();
		SetPhysics( &physicsObj );
		common->DPrintf( "TFPlayer::Spectate() - Disabling clip for %d '%s' - spectate: %d force: %d\n", entityNumber, GetUserInfo() ? GetUserInfo()->GetString( "ui_name" ) : "?", spectate, force );
...
/*
==============
TFPlayer::SetClipModel
==============
*/
void TFPlayer::SetClipModel( bool forceSpectatorBBox ) {
	idBounds bounds;

	common->DPrintf( "TFPlayer::SetClipModel() - Called on %d '%s' forceSpectatorBBox = %d spectate = %d instance = %d local instance = %d\n", entityNumber, GetUserInfo() ? GetUserInfo()->GetString( "ui_name" ) : "", forceSpectatorBBox, spectating, instance, gameLocal.GetLocalPlayer() ? gameLocal.GetLocalPlayer()->GetInstance() : -1 );
...
/*
==============
TFPlayer::EnterVehicle
==============
*/
bool TFPlayer::EnterVehicle( idEntity* vehicle ) {
...
/*
==============
TFPlayer::ExitVehicle
==============
*/
bool TFPlayer::ExitVehicle ( bool force ) {
...
==============
*/
int TFPlayer::GetItemCost( const char* itemName ) {
	if ( !itemCosts ) {
		assert( false );
		return 99999;
	}
...
bool TFPlayer::CanBuyItem( const char* itemName )
{
...
itemBuyStatus_t TFPlayer::ItemBuyStatus( const char* itemName )
{
...
/*
==============
TFPlayer::UpdateTeamPowerups
==============
*/
void TFPlayer::UpdateTeamPowerups( bool isBuying ) {
	for ( int i=0; i<MAX_TEAM_POWERUPS; i++ )
	{
		...
		if ( gameLocal.mpGame.teamPowerups[team][i].powerup != 0 && gameLocal.mpGame.teamPowerups[team][i].update )
		{
			...
			for( int j = 0; j < gameLocal.numClients; j++ ) {
				...
				if ( !ent->IsType( TFPlayer::Type ) )
					continue;

				TFPlayer* player = static_cast< TFPlayer * >( ent );
...
/*
==============
TFPlayer::AttemptToBuyTeamPowerup
==============
*/
bool TFPlayer::AttemptToBuyTeamPowerup( const char* itemName )
{
	idStr itemNameStr = itemName;
...
/*
==============
TFPlayer::AttemptToBuyItem
==============
*/
bool TFPlayer::AttemptToBuyItem( const char* itemName )
{
	assert( !IsFakeClient() );
...
void TFPlayer::GenerateImpulseForBuyAttempt( const char* itemName ) {
	if ( !CanBuy() )
		return;
...
/*
==============
TFPlayer::PerformImpulse
==============
*/
void TFPlayer::PerformImpulse( int impulse ) {
...
/*
==============
TFPlayer::HandleObjectiveInput
==============
*/
void TFPlayer::HandleObjectiveInput() {
#ifdef _XENON
	if ( gameLocal.inCinematic ) {
		return;
	}
...
/*
==============
TFPlayer::EvaluateControls
==============
*/
void TFPlayer::EvaluateControls( void ) {
...
/*
==============
TFPlayer::AdjustSpeed
==============
*/
void TFPlayer::AdjustSpeed( void ) {
	float speed;
...
/*
==============
TFPlayer::AdjustBodyAngles
==============
*/
void TFPlayer::AdjustBodyAngles( void ) {
	idMat3		lookAxis;
	idMat3		legsAxis;
	bool		blend;
	float		diff;
...
/*
==============
TFPlayer::InitAASLocation
==============
*/
void TFPlayer::InitAASLocation( void ) {
	int		i;
	int		num;
	idVec3	size;
	idBounds bounds;
...
/*
==============
TFPlayer::SetAASLocation
==============
*/
void TFPlayer::SetAASLocation( void ) {
	int		i;
	int		areaNum;
	idVec3	size;
	idBounds bounds;
...
/*
==============
TFPlayer::GetAASLocation
==============
*/
void TFPlayer::GetAASLocation( idAAS *aas, idVec3 &pos, int &areaNum ) const {
	int i;
...
void TFPlayer::Move( void ) {
	...
	if ( pfl.jump ) {
		...
		// (the AF becomes the active physics object, so physics don't run on TFPlayer_Physics and that flag never gets cleared)
		if ( health > 0 ) {
...
/*
==================
TFPlayer::BiasIntentDir

Called when a weapon fires, generates head twitches, etc
==================
*/
void TFPlayer::BiasIntentDir( idVec3 newIntentDir, float prevBias ) {
...
/*
==============
TFPlayer::UpdateIntentDir
==============
*/
void TFPlayer::UpdateIntentDir( void ) {
	idVec3 newIntentDir;
...
/*
==============
TFPlayer::UpdateDeathSkin
==============
*/
void TFPlayer::UpdateDeathSkin( bool state_hitch ) {
...
/*
===============
TFPlayer::LoadDeferredModel
===============
*/
void TFPlayer::LoadDeferredModel( void ) {
	if( !modelDecl ) {
		gameLocal.Warning( "TFPlayer::LoadDeferredModel() - reloadModel without vaid modelDict\n" );
		return;
	} 
...
/*
=================
TFPlayer::RouteGuiMouse
=================
*/
void TFPlayer::RouteGuiMouse( idUserInterface *gui ) {
 	sysEvent_t ev;
 	const char *command;
...
/*
==================
TFPlayer::LookAtKiller
==================
*/
void TFPlayer::LookAtKiller( idEntity *inflictor, idEntity *attacker ) {
	idVec3 dir;
...
==============
*/
void TFPlayer::Kill( bool delayRespawn, bool nodamage ) {
	if ( spectating ) {
		SpectateFreeFly( false );
	} else if ( health > 0 ) {
		godmode = false;
...
==================
*/
void TFPlayer::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	float delay;
	...
	if ( gameLocal.isMultiplayer ) {
		if ( gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() ) {
			if ( gameLocal.mpGame.GetGameState()->GetMPGameState() != WARMUP ) {
				...
				if ( attacker ) {
					TFPlayer* killer = NULL;
					if ( attacker->IsType( TFPlayer::Type ) ) {
						killer = static_cast<TFPlayer*>(attacker);
						if ( killer == this ) {
							// Killed by self
	...
	if ( gameLocal.isMultiplayer || g_testDeath.GetBool() ) {
		...
		if ( attacker->IsType( TFPlayer::Type ) ) {
			...
			if ( !isTelefragged ) {
				...
				} else if ( inflictor->IsType( TFPlayer::Type ) ) {
					// hitscan weapon
					methodOfDeath = static_cast<TFPlayer*>(inflictor)->GetCurrentWeapon();
				}
			}
...
// RAVEN END
	...
	if ( gameLocal.isMultiplayer && gameLocal.IsFlagGameType() ) {
		...
		if( PowerUpActive( POWERUP_CTF_STROGGFLAG ) || PowerUpActive( POWERUP_CTF_MARINEFLAG ) || PowerUpActive( POWERUP_CTF_ONEFLAG ) ) {
			TFPlayer* killer = (TFPlayer*)attacker;
			if ( killer != NULL && killer->IsType( TFPlayer::GetClassType() ) && killer != this ) {
				if ( killer->team != team ) {
...
=================
*/
void TFPlayer::CalcDamagePoints( idEntity *inflictor, idEntity *attacker, const idDict *damageDef,
	...
	TFPlayer *player = attacker->IsType( TFPlayer::Type ) ? static_cast<TFPlayer*>(attacker) : NULL;
	if ( !gameLocal.isMultiplayer ) {
...
============
*/
void TFPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	if ( damage > 0 ) {
		...
		if ( health <= 0 ) {
			...
			if ( oldHealth > 0 ) {	
				...
				if ( inflictor && inflictor->IsType ( TFPlayer::Type ) ) {
					pushScale = static_cast<TFPlayer*>(inflictor)->PowerUpModifier ( PMOD_PROJECTILE_DEATHPUSH );
				}
...
/*
=====================
TFPlayer::CanPlayImpactEffect 
=====================
*/
bool TFPlayer::CanPlayImpactEffect( idEntity* attacker, idEntity* target ) 
{
	if ( !gameLocal.isMultiplayer && attacker->IsType( idAI::GetClassType()) && target->IsType( TFPlayer::GetClassType())) 
	{
		// don't display impact effects when marines on our team shoot us...
		TFPlayer	*player = static_cast<TFPlayer *>( target );
...
/*
=====================
TFPlayer::AddDamageEffect
=====================
*/
void TFPlayer::AddDamageEffect( const trace_t &collision, const idVec3 &velocity, const char *damageDefName, idEntity* inflictor ) {
	if( gameLocal.isMultiplayer ) {
		if( ! cvarSystem->GetCVarBool("si_teamDamage") && inflictor && inflictor->IsType( TFPlayer::GetClassType() ) && gameLocal.IsTeamGame() && ((TFPlayer*)inflictor)->team == team ) {
			return;
		}
	}
...
============
*/
void TFPlayer::Teleport( const idVec3 &origin, const idAngles &angles, idEntity *destination ) {
 	idVec3 org;
...
/*
====================
TFPlayer::SetPrivateCameraView
====================
*/
void TFPlayer::SetPrivateCameraView( idCamera *camView ) {
...
====================
*/
float TFPlayer::DefaultFov( void ) const {
	float fov;
...
====================
*/
float TFPlayer::CalcFov( bool honorZoom ) {
	float fov;
...
/*
==============
TFPlayer::CalculateViewWeaponPos

Calculate the bobbing position of the view weapon
==============
*/
void TFPlayer::CalculateViewWeaponPos( idVec3 &origin, idMat3 &axis ) {
	float		scale;
	float		fracsin;
	idAngles	angles;
	int			delta;
	...
	const TFPlayer* player = gameLocal.GetLocalPlayer();
...
/*
===============
TFPlayer::OffsetThirdPersonVehicleView
===============
*/
// RAVEN BEGIN
// jnewquist: option to avoid clipping against world
void TFPlayer::OffsetThirdPersonVehicleView( bool clip ) {
// RAVEN END
	idVec3			view;
	idVec3			focusAngles;
	trace_t			trace;
...
/*
===============
TFPlayer::OffsetThirdPersonView
===============
*/
void TFPlayer::OffsetThirdPersonView( float angle, float range, float height, bool clip ) {
	idVec3			view;
	idVec3			focusAngles;
	trace_t			trace;
	idVec3			focusPoint;
...
/*
===============
TFPlayer::GetEyePosition
===============
*/
idVec3 TFPlayer::GetEyePosition( void ) const {
	idVec3 org;
...
===============
*/
void TFPlayer::GetViewPos( idVec3 &origin, idMat3 &axis ) const {
	idAngles angles;
...
/*
===============
TFPlayer::CalculateFirstPersonView
===============
*/
void TFPlayer::CalculateFirstPersonView( void ) {
...
/*
==================
TFPlayer::GetRenderView

Returns the renderView that was calculated for this tic
==================
*/
renderView_t *TFPlayer::GetRenderView( void ) {
	return renderView;
}

/*
==================
TFPlayer::SmoothenRenderView
...
==================
*/
void TFPlayer::SmoothenRenderView( bool firstPerson ) {
	int d1, d2;
...
/*
==================
TFPlayer::CalculateRenderView

create the renderView for the current tic
==================
*/
void TFPlayer::CalculateRenderView( void ) {
	int i;
	float range;
...
/*
==================
TFPlayer::Event_EnableTarget
==================
*/
void TFPlayer::Event_EnableTarget ( void ) {
	fl.notarget = false;
}

/*
==================
TFPlayer::Event_DisableTarget
==================
*/
void TFPlayer::Event_DisableTarget ( void ) {
	fl.notarget = true;
}

/*
==================
TFPlayer::Event_GetViewPos
==================
*/
void TFPlayer::Event_GetViewPos( void ) {
	idVec3		viewOrigin;
	idMat3		viewAxis;
...
/*
==================
TFPlayer::Event_FinishHearingLoss
==================
*/
void TFPlayer::Event_FinishHearingLoss ( float fadeTime ) {
	if ( fadeTime <= 0.0f ) {
...
/*
=============
TFPlayer::Event_ApplyImpulse
=============
*/
void TFPlayer::Event_ApplyImpulse ( idEntity* ent, idVec3 &point, idVec3 &impulse ) {
...
/*
=============
TFPlayer::Event_EnableObjectives
=============
*/
void TFPlayer::Event_EnableObjectives ( void ) {
	objectivesEnabled = true;
}
...
/*
=============
TFPlayer::Event_DisableObjectives
=============
*/
void TFPlayer::Event_DisableObjectives ( void ) {
...
void TFPlayer::Event_AllowNewObjectives ( void ) {
...
/*
=============
TFPlayer::Event_SetHealth
=============
*/
void TFPlayer::Event_SetHealth( float newHealth ) {
	health = idMath::ClampInt( 1 , inventory.maxHealth, newHealth );
}
/*
=============
TFPlayer::Event_SetArmor
=============
*/
void TFPlayer::Event_SetArmor( float newArmor ) {
	inventory.armor = idMath::ClampInt( 0 , inventory.maxarmor, newArmor );
}

/*
=============
TFPlayer::Event_SetExtraProjPassEntity
=============
*/
void TFPlayer::Event_SetExtraProjPassEntity( idEntity* _extraProjPassEntity ) {
...
/*
=============
TFPlayer::AddProjectilesFired
=============
*/
void TFPlayer::AddProjectilesFired( int count ) {
	numProjectilesFired += count;
}

/*
=============
TFPlayer::AddProjectileHites
=============
*/
void TFPlayer::AddProjectileHits( int count ) {
	numProjectileHits += count;
}

/*
=============
TFPlayer::TriggerHitSound
=============
*/
void TFPlayer::TriggerHitSound( bool armor ) {
...
/*
=============
TFPlayer::SetInfluenceLevel
=============
*/
void TFPlayer::SetInfluenceLevel( int level ) {
...
/*
=============
TFPlayer::SetInfluenceView
=============
*/
void TFPlayer::SetInfluenceView( const char *mtr, const char *skinname, float radius, idEntity *ent ) {
	influenceMaterial = NULL;
	influenceEntity = NULL;
	influenceSkin = NULL;
	if ( mtr && *mtr ) {
...
/*
=============
TFPlayer::SetInfluenceFov
=============
*/
void TFPlayer::SetInfluenceFov( float fov ) {
	influenceFov = fov;
}
...
================
*/
bool TFPlayer::OnLadder( void ) const {
	return physicsObj.OnLadder();
}

/*
==================
TFPlayer::Event_GetButtons
==================
*/
void TFPlayer::Event_GetButtons( void ) {
	idThread::ReturnInt( usercmd.buttons );
}

/*
==================
TFPlayer::Event_GetMove
==================
*/
void TFPlayer::Event_GetMove( void ) {
...
/*
================
TFPlayer::Event_GetViewAngles
================
*/
void TFPlayer::Event_GetViewAngles( void ) {
	idThread::ReturnVector( idVec3( viewAngles[0], viewAngles[1], viewAngles[2] ) );
}

/*
================
TFPlayer::Event_SetViewAngles
================
*/
void TFPlayer::Event_SetViewAngles( const idVec3 & vec ) {
	idAngles ang;
...
/*
==================
TFPlayer::Event_StopFxFov
==================
*/
void TFPlayer::Event_StopFxFov( void ) {
	fxFov = false;
}
...
==================
*/
void TFPlayer::StartFxFov( float duration ) { 
	fxFov = true;
...
/*
==================
TFPlayer::Event_EnableWeapon 
==================
*/
void TFPlayer::Event_EnableWeapon( void ) {
...
/*
==================
TFPlayer::Event_DisableWeapon
==================
*/
void TFPlayer::Event_DisableWeapon( void ) {
 	hiddenWeapon = true;
	weaponEnabled = false;
   	if ( weapon ) {
		weapon->EnterCinematic();
...
/*
==================
TFPlayer::Event_GetCurrentWeapon
==================
*/
void TFPlayer::Event_GetCurrentWeapon( void ) {
	if ( currentWeapon >= 0 ) {
...
/*
==================
TFPlayer::Event_GetPreviousWeapon
==================
*/
void TFPlayer::Event_GetPreviousWeapon( void ) {
	if ( previousWeapon >= 0 ) {
...
/*
==================
TFPlayer::Event_SelectWeapon
==================
*/
void TFPlayer::Event_SelectWeapon( const char *weaponName ) {
	int i;
	int weaponNum;
...
/*
==================
TFPlayer::Event_GetAmmoData
==================
*/
void TFPlayer::Event_GetAmmoData( const char *ammoClass ) {
...
/*
==================
TFPlayer::Event_RefillAmmo
==================
*/
void TFPlayer::Event_RefillAmmo( void ) {
	int a;
	for ( int i = 0; i < MAX_WEAPONS; i++ ) {
		if ( inventory.weapons & ( 1 << i ) ) {
			const char *weap = spawnArgs.GetString( va( "def_weapon%d", i ) );
...
/*
==================
TFPlayer::Event_AllowFallDamage
==================
*/
void TFPlayer::Event_AllowFallDamage( int toggle ) {
	if( toggle )	{
		pfl.noFallingDamage = false;
	} else {
...
/*
==================
TFPlayer::Event_GetWeaponEntity
==================
*/
void TFPlayer::Event_GetWeaponEntity( void ) {
	idThread::ReturnEntity( weaponViewModel );
}

/*
==================
TFPlayer::Event_HideDatabaseEntry
==================
*/
void TFPlayer::Event_HideDatabaseEntry ( void ) {
	if ( hud ) {
...
/*
==================
TFPlayer::Event_ZoomIn
==================
*/
void TFPlayer::Event_ZoomIn ( void ) {
	float currentFov;
	float t;
...
/*
==================
TFPlayer::Event_ZoomOut
==================
*/
void TFPlayer::Event_ZoomOut ( void ) {
	float t;
	float currentFov;
...
/*
==================
TFPlayer::TeleportDeath
==================
*/
void TFPlayer::TeleportDeath( int killer ) {
	teleportKiller = killer;
}

/*
==================
TFPlayer::Event_ExitTeleporter
==================
*/
void TFPlayer::Event_ExitTeleporter( void ) {
	idEntity	*exitEnt;
	float		pushVel;
...
/*
===============
TFPlayer::Event_DamageOverTimeEffect
===============
*/
void TFPlayer::Event_DamageOverTimeEffect( int endTime, int interval, const char *damageDefName ) {
	const idDeclEntityDef *damageDef = gameLocal.FindEntityDef( damageDefName, false );
	if ( damageDef ) {
...
/*
===============
TFPlayer::LocalClientPredictionThink
===============
*/
void TFPlayer::LocalClientPredictionThink( void ) {
	renderEntity_t *headRenderEnt;
...
/*
===============
TFPlayer::NonLocalClientPredictionThink
===============
*/
#define LIMITED_PREDICTION		1

void TFPlayer::NonLocalClientPredictionThink( void ) {
	assert( !IsFakeClient() );
...
/*
================
TFPlayer::ClientPredictionThink
================
*/
void TFPlayer::ClientPredictionThink( void ) {	
	...
	TFPlayer *p = gameLocal.GetClientByNum( gameLocal.localClientNum );
	if ( p && p->spectating && p->spectator == entityNumber ) {
		LocalClientPredictionThink();
		return;
	}
...
/*
================
TFPlayer::GetMasterPosition
================
*/
bool TFPlayer::GetMasterPosition( idVec3 &masterOrigin, idMat3 &masterAxis ) const {
	if( !IsInVehicle() ) {
...
/*
===============
TFPlayer::PredictionErrorDecay
===============
*/
void TFPlayer::PredictionErrorDecay( void ) {
	if ( predictedUpdated ) {
		return;
	}
...
/*
===============
TFPlayer::WantSmoothing
===============
*/
bool TFPlayer::WantSmoothing( void ) const {
	if ( !gameLocal.isClient ) {
		return false;
	}
	if ( net_predictionErrorDecay.GetFloat() <= 0.0f ) {
...
/*
================
TFPlayer::GetPhysicsToVisualTransform
================
*/
bool TFPlayer::GetPhysicsToVisualTransform( idVec3 &origin, idMat3 &axis ) {
	if ( af.IsActive() ) {
...
/*
================
TFPlayer::GetPhysicsToSoundTransform
================
*/
bool TFPlayer::GetPhysicsToSoundTransform( idVec3 &origin, idMat3 &axis ) {
	idCamera *camera;
...
/*
================
TFPlayer::WriteToSnapshot
================
*/
void TFPlayer::WriteToSnapshot( idBitMsgDelta &msg ) const {
	assert( !IsFakeClient() );
...
/*
================
TFPlayer::ReadFromSnapshot
================
*/
void TFPlayer::ReadFromSnapshot( const idBitMsgDelta &msg ) {
	assert( !IsFakeClient() );
	...
	if ( spectating && latchedSpectator != spectator ) {
		...
		if ( this == gameLocal.GetLocalPlayer() ) {
			...
			if ( gameLocal.entities[ spectator ] ) {
				TFPlayer *p = static_cast< TFPlayer * >( gameLocal.entities[ spectator ] );
	...
	if( lastKillerEntity >= 0 && lastKillerEntity < MAX_CLIENTS)	{
		lastKiller = static_cast<TFPlayer *>(gameLocal.entities[ lastKillerEntity ]);
	} else {
		lastKiller = NULL;
	}
	...
	if ( oldHealth > 0 && health <= 0 ) {
		...
		if ( weapon ) {
 		...
 		// respawn
		//common->DPrintf( "TFPlayer::ReadFromSnapshot() - Player respawn detected for %d '%s' - re-enabling clip\n", entityNumber, GetUserInfo() ? GetUserInfo()->GetString( "ui_name" ) : "" );
...
/*
================
TFPlayer::WritePlayerStateToSnapshot
================
*/
void TFPlayer::WritePlayerStateToSnapshot( int lastSnapshotFrame, idBitMsgDelta &msg ) const {
	int i;
...
/*
================
TFPlayer::ReadPlayerStateFromSnapshot
================
*/
void TFPlayer::ReadPlayerStateFromSnapshot( const idBitMsgDelta &msg ) {
	int i, ammo;
...
/*
================
TFPlayer::ServerReceiveEvent
================
*/
bool TFPlayer::ServerReceiveEvent( int event, int time, const idBitMsg &msg ) {
...
/*
===============
TFPlayer::CheckAckReply
===============
*/
void TFPlayer::CheckAckReply( void ) const {
	assert( !gameLocal.isClient );
...
/*
================
TFPlayer::ClientReceiveEvent
================
*/
bool TFPlayer::ClientReceiveEvent( int event, int time, const idBitMsg &msg ) {
 	int powerup;
	bool start;
...
===============
*/
void TFPlayer::ShowTip( const char *title, const char *tip, bool autoHide ) {
	if ( tipUp ) {
		return;
	}
...
/*
===============
TFPlayer::Event_HideTip
===============
*/
void TFPlayer::Event_HideTip( void ) {
	HideTip();
}

/*
===============
TFPlayer::ShowObjective
===============
*/
void TFPlayer::ShowObjective( const char *obj ) {
...
/*
===============
TFPlayer::HideObjective
===============
*/
void TFPlayer::HideObjective( void ) {
...
/*
===============
TFPlayer::SetSpectateOrigin
===============
*/
void TFPlayer::SetSpectateOrigin( void ) {
	idVec3 neworig;
...
/*
===============
TFPlayer::RemoveWeapon
===============
*/
void TFPlayer::RemoveWeapon( const char *weap ) {
	if ( weap && *weap ) {
...
/*
===============
TFPlayer::CanShowWeaponViewmodel
===============
*/
bool TFPlayer::CanShowWeaponViewmodel( void ) const {
 	return showWeaponViewModel;
}

/*
===============
TFPlayer::SetLevelTrigger
===============
*/
void TFPlayer::SetLevelTrigger( const char *levelName, const char *triggerName ) {
...
/*
===============
TFPlayer::Event_LevelTrigger
===============
*/
void TFPlayer::Event_LevelTrigger( void ) {
...
/*
================
TFPlayer::ToggleFlashlight
================
*/
void TFPlayer::ToggleFlashlight ( void ) {
...
================
*/
void TFPlayer::Flashlight ( bool on ) {
	flashlightOn = on;	
}

/*
================
TFPlayer::DamageFeedback
================
*/
void TFPlayer::DamageFeedback( idEntity *victim, idEntity *inflictor, int &damage ) {
	...
	if ( gameLocal.isMultiplayer && victim->IsType( TFPlayer::GetClassType() ) ) {
		if ( this == victim ) {
			// no feedback for self hits
			return;
		}
		TFPlayer *p = static_cast< TFPlayer * >( victim );
		if ( gameLocal.IsTeamGame() && p->team == team ) {
...
/*
==============
TFPlayer::GetWeaponDef
==============
*/
const idDeclEntityDef* TFPlayer::GetWeaponDef ( int weaponIndex ) {
...
==============
*/
const idDeclEntityDef* TFPlayer::GetPowerupDef ( int powerupIndex ) {
	const idDict* types;
	int			  i;
	int			  num;
...
/*
==============
TFPlayer::discoverSecretArea

Announces a secret area and increases the secret area tally
==============
*/
void TFPlayer::DiscoverSecretArea(const char* _description)	{
...
/*
=====================
TFPlayer::SetInitialHud
=====================
*/
void TFPlayer::SetInitialHud ( void ) {
...
void TFPlayer::RemoveClientModel ( const char *entityDefName ) {
	rvClientEntity* cent;
	rvClientEntity*	next;
...
rvClientEntityPtr<rvClientModel> TFPlayer::AddClientModel ( const char* entityDefName, const char* shaderName ) {
...
void TFPlayer::RemoveClientModels ( void ) {
	rvClientEntity* cent;
	rvClientEntity*	next;
...
=====================
*/
void TFPlayer::ClientGib( const idVec3& dir ) {
...
=====================
*/
bool TFPlayer::CanDamage( const idVec3 &origin, idVec3 &damagePoint, idEntity *ignoreEnt ) {
...
/*
=====================
TFPlayer::ClientDamageEffects

=====================
*/
void TFPlayer::ClientDamageEffects ( const idDict& damageDef, const idVec3& dir, int damage ) {
	idVec3 from;
	idVec3 localDir;
	float  fadeDB;
...
/*
=====================
TFPlayer::GetDebugInfo
=====================
*/
void TFPlayer::GetDebugInfo ( debugInfoProc_t proc, void* userData ) {
	idActor::GetDebugInfo ( proc, userData );
	proc ( "TFPlayer", "inventory.armor",		va("%d", inventory.armor ), userData );
	proc ( "TFPlayer", "inventory.weapons",		va("%d", inventory.weapons ), userData );
	proc ( "TFPlayer", "inventory.powerups",	va("%d", inventory.powerups ), userData );
}
...
/*
=====================
TFPlayer::ApplyImpulse
=====================
*/
void TFPlayer::ApplyImpulse( idEntity *ent, int id, const idVec3 &point, const idVec3 &impulse, bool splash ) {
	if( !ent)	{
		gameLocal.Warning( "TFPlayer::ApplyImpulse called with null entity as instigator.");
		return;
	}
	...
	if( ent->IsType( TFPlayer::Type ) && ent != this ) {
		lastImpulsePlayer = static_cast<TFPlayer*>(ent);
// RAVEN BEGIN
// jnewquist: Use accessor for static class type 
	} else if( ent->IsType( idProjectile::GetClassType() ) ) {
// RAVEN END
		idEntity* owner = static_cast<idProjectile*>(ent)->GetOwner();
		if( owner && owner->IsType( TFPlayer::Type ) && owner != this ) {
			lastImpulsePlayer = static_cast<TFPlayer*>(owner);
		}
	}
...
=====================
*/
void TFPlayer::SetupHead( const char* headModel, idVec3 headOffset ) {
	if( gameLocal.isMultiplayer ) {
...
/*
=====================
TFPlayer::GUIMainNotice
=====================
*/
void TFPlayer::GUIMainNotice( const char* message, bool persist ) {
...
/*
=====================
TFPlayer::GUIFragNotice
=====================
*/
void TFPlayer::GUIFragNotice( const char* message, bool persist ) {
	if( !gameLocal.isMultiplayer || !mphud ) {
		return;
	}
...
/*
=====================
TFPlayer::SetHudOverlay
=====================
*/
void TFPlayer::SetHudOverlay( idUserInterface* overlay, int duration ) {
	overlayHud = overlay;
...
/*
=====================
TFPlayer::SaveMessage
=====================
*/
void TFPlayer::SaveMessage( void ) {
#ifndef _XENON
	if ( GetHud( ) ) {
...
/*
=====================
TFPlayer::GetSpawnClassname
=====================
*/
const char* TFPlayer::GetSpawnClassname ( void ) {
	idEntity*	world;
...
/*
===============
TFPlayer::SetInstance
===============
*/
void TFPlayer::SetInstance( int newInstance ) {
	common->DPrintf( "TFPlayer::SetInstance() - Setting instance for '%s' to %d\n", name.c_str(), newInstance );
	...
	if( this == gameLocal.GetLocalPlayer() ) {
		...
		if( gameLocal.isServer ) {
			...
			for( int i = 0; i < MAX_CLIENTS; i++ ) {
				TFPlayer* player = (TFPlayer*)gameLocal.entities[ i ];
				if( player ) {
...
/*
===============
TFPlayer::JoinInstance
===============
*/
void TFPlayer::JoinInstance( int newInstance ) {
	...
	if( newInstance < 0 || newInstance >= MAX_ARENAS ) {
		gameLocal.Warning( "TFPlayer::JoinInstance() - Invalid instance %d specified\n", newInstance );
	}
...
===============
*/
void TFPlayer::SetEmote( playerEmote_t newEmote ) {
	emote = newEmote;
...
/*
===============
TFPlayer::GetGroundElevator
===============
*/
idEntity* TFPlayer::GetGroundElevator( idEntity* testElevator ) const {
...
/*
===================
TFPlayer::IsCrouching
===================
*/
bool TFPlayer::IsCrouching( void ) const {
...
===============
*/
void TFPlayer::SetArena( int newArena ) {
	if( arena == newArena ) {
		return;
	}
...
/*
===============
TFPlayer::Event_DamageEffect
===============
*/
void TFPlayer::Event_DamageEffect( const char *damageDefName, idEntity* _damageFromEnt )
{ 
	const idDeclEntityDef *damageDef = gameLocal.FindEntityDef( damageDefName, false );
	if ( damageDef )
	{
...
/*
===============
TFPlayer::UpdateDeathShader
===============
*/
void TFPlayer::UpdateDeathShader ( bool state_hitch ) {
...
/*
===============
TFPlayer::Event_InitWeapon
===============
*/
void TFPlayer::InitWeapon( void ) {
	currentWeapon = -1;
	SetWeapon( idealWeapon );
}
#endif
...
/*
===============
TFPlayer::GetHitscanTint
===============
*/
const idVec4& TFPlayer::GetHitscanTint( void ) {
	assert( !IsFakeClient() );
	...
	if( gameLocal.IsTeamGame() ) {
		if( gameLocal.serverInfo.GetInt( "si_allowHitscanTint" ) >= 2 ) {
			if( team == TEAM_MARINE ) {
				...
				gameLocal.Error( "TFPlayer::GetHitscanTint() - Unknown team '%d' on player %d '%s'\n", team, entityNumber, GetUserInfo()->GetString( "ui_name" ) );
			}
		} else {
...
===============
*/
bool TFPlayer::IsReady( void ) {
	return !gameLocal.serverInfo.GetBool( "si_useReady" ) || ready || forcedReady;
}

/*
===============
TFPlayer::ForceScoreboard
===============
*/
void TFPlayer::ForceScoreboard( bool force, int time ) {
...
/*
===============
TFPlayer::GetTextTourneyStatus
===============
*/
const char* TFPlayer::GetTextTourneyStatus( void ) {
...
===============
*/
void TFPlayer::ClientInstanceJoin( void ) {
	assert( gameLocal.isClient );

	common->DPrintf( "TFPlayer::ClientInstanceJoin() - client %d ('%s') is being shown\n", entityNumber, GetUserInfo() ? GetUserInfo()->GetString( "ui_name" ) : "?" );
...
/*
===============
TFPlayer::ClientInstanceLeave
Players know about all other players, even in other instances
We need to hide/show them on the client as we switch to/from instances
===============
*/
void TFPlayer::ClientInstanceLeave( void ) {
	assert( gameLocal.isClient );
	common->DPrintf( "TFPlayer::ClientInstanceLeave() - client %d ('%s') is being hidden\n", entityNumber, GetUserInfo() ? GetUserInfo()->GetString( "ui_name" ) : "?" );
...
/*
===============
TFPlayer::ClientStale
===============
*/
bool TFPlayer::ClientStale( void ) {
...
/*
===============
TFPlayer::ClientUnstale
===============
*/
void TFPlayer::ClientUnstale( void ) {
...
/*
===============
TFPlayer::AllowedVoiceDest
===============
*/
bool TFPlayer::AllowedVoiceDest( int from ) {
...
// RITUAL BEGIN
void TFPlayer::ClampCash( float minCash, float maxCash )
{
...
void TFPlayer::GiveCash( float cashDeltaAmount )
{
...
void TFPlayer::SetCash( float newCashAmount )
{
	//int minCash = gameLocal.mpGame.mpBuyingManager.GetIntValueForKey( "playerMinCash", 0 );
	//int maxCash = gameLocal.mpGame.mpBuyingManager.GetIntValueForKey( "playerMaxCash", 0 );
	float minCash = (float) gameLocal.serverInfo.GetInt("si_buyModeMinCredits");
 ...
 */
int TFPlayer::CanSelectWeapon(const char* weaponName)
{
	int weaponNum = -1;
	if(weaponName == NULL)
		return weaponNum;
...
bool TFPlayer::IsSpectatedClient( void ) const {
	TFPlayer *localPlayer = gameLocal.GetLocalPlayer();
	if ( !localPlayer ) {
		return false;
	}
